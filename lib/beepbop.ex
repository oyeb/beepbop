  ...
  * `:ecto_repo` -- Since BeepBop does the routine persisting of "state", it
  ...
  defmacro event(event, options, callback) do
    quote location: :keep do
      ...
      @spec unquote(event)(map | context, keyword) :: {:ok, map | context} | {:error, term}
            when context: State.t()
      def unquote(event)(context, opts \\ [persist: true])

      def unquote(event)(%State{} = context, opts) do
      ...
      def unquote(event)(context, opts) when is_map(context) do
        beepbop_context = State.new(context)
  ...
  def context_validator(schema) do
    quote location: :keep do
      ...
      """
      @spec valid_context?(State.t()) :: boolean
      def valid_context?(context)

      def valid_context?(%State{
  ...
  def persist_helpers do
    quote location: :keep do
      ...
      defp __beepbop_try_persist(%State{valid?: false} = context, _, _) do
        context
      end

      defp __beepbop_try_persist(%State{valid?: true} = context, to_state, opts) do
        ...
        if persist? do
          ...
          State.new(context.context, struct, final_multi)
        end
      end
    end
  end
  ...
  defmacro __before_compile__(env) do
    ...
    quote location: :keep do
      ...
      valid.
      """
      @spec can_transition?(State.t(), atom) :: boolean
